...
export default function PaymentPage() {
  ...
  const selectedPackage = useMemo(() => {
    return packages[(packageType as keyof typeof packages)] ?? packages.standard;
  }, [packageType]);
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setBuyerPhone(formatPhoneNumber(e.target.value));
  };
  ...
  const requestPayment = (orderNumber: string) => {
    ...
    window.INIStdPay.pay({
      ...
      onError: function (error: any) {
        alert("결제 중 오류가 발생했습니다: " + (error?.message ?? "unknown"));
        router.push(`/payment-fail?reason=${encodeURIComponent(error?.message ?? "unknown")}`);
      },
    });
  };
      ...
      <div className="max-w-[500px] mx-auto my-10 bg-white rounded-[20px] shadow-2xl overflow-hidden">
        ...
        <div className="p-10">
          <div className="bg-[#f8f9fa] rounded-xl p-6 mb-8 border-2 border-[#e9ecef]">
            <div className="text-2xl font-bold text-[#667eea] mb-4">{selectedPackage.name}</div>
